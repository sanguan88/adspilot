...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      const roleFilter = getRoleBasedFilter(user.role, user.userId, user.username)
      
      // Query untuk mendapatkan semua toko dengan cookies
      const query = `
        SELECT 
          dt.id_toko,
          dt.nama_toko,
        ...
        FROM data_toko dt
        WHERE dt.status_toko = 'active'
          ${roleFilter.whereClause}
        ORDER BY dt.nama_toko ASC
      `
      
      const params = roleFilter.paramsCount > 0 ? [user.userId] : []
      ...
      const healthStatus = tokos.map((toko: any) => {
        ...
        if (!hasCookies) {
          ...
          if (diffHours < 24) {
            ...
            health = 'expired' // > 3 hari = kemungkinan expired
            needsUpdate = true
          }
        }
