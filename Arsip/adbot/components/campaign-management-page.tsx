...
import { Search, Filter, Plus, MoreHorizontal, BarChart3, Loader2, AlertCircle, X, Play, Pause, Square, Ban, PlayCircle, PauseCircle, CheckCircle, RefreshCw, CalendarIcon, ArrowUpDown, ArrowUp, ArrowDown, Info, Download, Store } from "lucide-react"
...
export function CampaignManagementPage() {
  ...
  useEffect(() => {
    const checkAutomaticCampaign = async () => {
      ...
      try {
        ...
        const response = await authenticatedFetch(`/api/campaigns/shopee?start_time=${dateRange.startTime}&end_time=${dateRange.endTime}&account_ids=${account.username}`)
  ...
  }, [createFormData.account_username, connectedAccounts, dateRange.startTime, dateRange.endTime])
  ...
  // Function to handle date range change (only validation, no fetch)
  const handleDateRangeChange = () => {
    // Validate dates (but don't fetch - wait for Apply Filter button)
    const today = new Date().toISOString().split('T')[0]
    const startDate = new Date(dateRange.startTime)
    const endDate = new Date(dateRange.endTime)
    const todayDate = new Date(today)
    ...
    // Don't fetch here - wait for Apply Filter button click
  }
  ...
  const handleCreateCampaignSubmit = async () => {
    ...
    if (createFormData.mode === 'automatic') {
      ...
      // Validasi kelipatan 5000 (hanya jika budget > 0)
    if (budget > 0 && budget % 5 !== 0) {
        toast.error('Validasi Gagal', {
          description: 'Budget harus kelipatan Rp5.000'
        })
        return
    }
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Manajemen Iklan</h1>
                  ...
                  <Popover open={startDateOpen} onOpenChange={setStartDateOpen}>
                    <PopoverTrigger asChild>
                      <button
                        type="button"
                        className={cn(
                          "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-sm font-medium transition-all h-9 gap-1.5 px-3 text-xs",
                      ...
                      >
                        ...
                        {dateRange.startTime ? (
                          format(new Date(dateRange.startTime + 'T00:00:00'), "MMM dd, yyyy")
                        ) : (
                  ...
                  <Popover open={endDateOpen} onOpenChange={setEndDateOpen}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        <CalendarIcon className="mr-2 h-4 w-4 shrink-0" />
                        {dateRange.endTime ? (
                          format(new Date(dateRange.endTime + 'T00:00:00'), "MMM dd, yyyy")
                        ) : (
                          <span>Pick a date</span>
                        )}
                      </button>
              ...
              >
                ...
                <thead className="sticky top-0 z-30">
                      <tr className="border-b bg-gray-50">
                        <th className="text-left py-3 px-4 font-medium text-sm sticky left-0 z-30 bg-gray-50" style={{ boxShadow: '2px 0 4px rgba(0,0,0,0.1)' }}>
                ...
                <tbody>
                      {paginatedCampaigns.map((campaign) => {
                        ...
                        <tr key={campaign.id} className="border-b hover:bg-gray-50">
                          <td className="py-3 px-4 sticky left-0 z-10 bg-white" style={{ boxShadow: '2px 0 4px rgba(0,0,0,0.1)' }}>
                            <div className="flex items-center gap-3">
                              {/* Status Icon */}
                          ...
                          <td className="py-3 px-4 text-right text-sm">
                            {((campaign.ctr || 0) > 0 ? (campaign.ctr || 0).toFixed(1) : '0.0')}%
                          </td>
                          <td className="py-3 px-4 text-right text-sm">
                          ...
                          <td className="py-3 px-4 text-center sticky right-0 z-10 bg-white" style={{ boxShadow: '-2px 0 4px rgba(0,0,0,0.1)' }}>
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <Button
                                ...
                                onClick={() => handleOpenOperationLog(campaign.id, campaign.account_username)}
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {createFormData.mode === 'automatic' 
                    ? 'Budget: 0 (tak terbatas) atau minimal Rp50.000, harus kelipatan Rp5.000'
                    : 'Budget: 0 (tak terbatas) atau minimal Rp25.000, harus kelipatan Rp5.000'}
                </p>
              </div>
            </div>
          </div>
