...
export function ConditionBuilder({ conditions, onChange }: ConditionBuilderProps) {
  ...
  return (
    <div className="space-y-4">
      {conditions.map((group, groupIndex) => (
        <Card key={group.id} className="p-4">
          <div className="space-y-4">
                    ...
                    {conditionIndex > 0 && (
                      <Badge
                        ...
                        className={cn(
                          ...
                          group.operator === "AND" ? "bg-blue-100 text-blue-800" : "bg-green-100 text-green-800",
                        )}
                      >
                        {group.operator}
                      </Badge>
            ...
            <Button variant="ghost" size="sm" onClick={() => addCondition(group.id)} className="text-blue-600">
      ...
      {/* Add Group Button */}
      <Button variant="ghost" size="sm" onClick={addConditionGroup} className="text-blue-600">
        <Plus className="w-4 h-4 mr-2" />
        Add Group
      </Button>
      ...
      {conditions.length > 0 && (
        <Card className="p-4 bg-gray-50">
          ...
          <div className="text-sm text-gray-600 space-y-1">
            {conditions.map((group, groupIndex) => (
              <div key={group.id}>
                {groupIndex > 0 && <span className="font-medium text-blue-600">OR</span>}
                <div className="ml-4">
                  {group.conditions.map((condition, conditionIndex) => {
                    ...
                    return (
                      <div key={condition.id} className="flex items-center gap-2">
                        {conditionIndex > 0 && (
                          <span
                            className={cn("font-medium", group.operator === "AND" ? "text-blue-600" : "text-green-600")}
                          >
                            {group.operator}
                          </span>
                        )}
