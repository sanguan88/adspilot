...
export async function GET(request: NextRequest) {
  try {
    ...
    const [timRows] = await connection.execute(`
    ...
    const [picRows] = await connection.execute(`
    ...
    const dataAkun = rows as any[]
    const timData = timRows as any[]
    const picData = picRows as any[]
