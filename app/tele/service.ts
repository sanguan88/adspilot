...
import { formatRuleNotificationMessage, replaceTemplateVariables, isValidChatId, escapeMarkdown } from './utils'
...
export async function sendRuleNotification(
  ...
  if (data.message) {
    ...
    message = replaceTemplateVariables(data.message, {
      ruleName: escapeMarkdown(data.ruleName),
      ruleId: escapeMarkdown(data.ruleId),
      time: data.triggeredAt,
      action: data.actions.map(a => escapeMarkdown(a)).join(', '),
    })
  } else {
