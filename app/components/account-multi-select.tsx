...
import { Users, Check, ChevronDown, Loader2, X, AlertTriangle } from "lucide-react"
...
export function AccountMultiSelect({
  ...
  const handleToggleAccount = (accountId: string) => {
    // Prevent selecting expired accounts
    const health = tokos[accountId]
    if (health === 'expired' || health === 'no_cookies') return
    ...
    if (checked) {
      const allIds = filteredAccounts
        ...
        .filter(id => {
          ...
          return id && id !== 'null' && id !== 'undefined' && !isExpired
        })
                ...
                <CommandGroup>
                  ...
                  {filteredAccounts.map((account) => {
                    ...
                    return (
                      <CommandItem
                        key={accountId}
                        onSelect={() => !isExpired && handleToggleAccount(accountId)}
                        disabled={isExpired}
                        className={cn("cursor-pointer", isExpired && "opacity-50 cursor-not-allowed")}
                      >
                        <Checkbox
                          checked={isSelected}
                          onCheckedChange={() => !isExpired && handleToggleAccount(accountId)}
                          disabled={isExpired}
