...
export function isDatabaseConnectionError(error: any): boolean {
  ...
  const hasDbKeyword = dbKeywords.some(keyword => {
    // Special case for 'authentication': exclude 'Authentication required' which is a user auth error
    if (keyword === 'authentication' && errorMessage.includes('authentication required')) {
      return false;
    }
