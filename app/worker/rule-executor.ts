...
import { fetchCampaignData, fetchAllCampaignsData, getTokoName } from './shopee-api-client'
...
async function processCampaignsForToko(
  ...
  try {
    ...
    const campaignNamesToNotify: string[] = [] // Names of campaigns that met conditions
    ...
    for (const campaignId of matchingCampaigns) {
      ...
      if (evalResult.passed) {
        ...
        campaignNamesToNotify.push(campaignData.campaign_name || campaignId)
    ...
    logger.info(
      `[Rule: ${rule.name}] Notification check: enabled=${rule.telegram_notification}, ` +
      `campaigns_met=${campaignsToExecute.length}, successful_executions=${successfulCampaigns}, ` +
      `should_send=${shouldSendNotification}`
    )
    ...
    if (shouldSendNotification) {
      ...
      if (chatId) {
        logger.info(`[Rule: ${rule.name}] Found Chat ID ${chatId} for user ${rule.user_id}`)
        const tokoName = await getTokoName(tokoId) || tokoId
        ...
        try {
          await sendRuleNotification(chatId, {
            ...
            campaignNames: campaignNamesToNotify,
          })
          logger.info(
      ...
      logger.info(
        `[Rule: ${rule.name}] Telegram notification skipped logic. ` +
