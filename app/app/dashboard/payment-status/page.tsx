...
export default function PaymentStatusPage() {
  ...
  const handleContactAdmin = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
  ...
  const handleUploadProof = async () => {
    ...
    try {
      ...
      if (user?.userId) {
        const transactionResponse = await fetch(`/api/transactions?limit=1&userId=${user.userId}`)
        const transactionResult = await transactionResponse.json()
        if (transactionResult.success && transactionResult.data && transactionResult.data.length > 0) {
          setTransaction(transactionResult.data[0])
        }
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    if (transaction?.paymentStatus === 'pending' || transaction?.paymentStatus === 'waiting_confirmation') {
      const interval = setInterval(async () => {
        ...
        try {
          const response = await fetch(`/api/transactions?limit=1&userId=${user.userId}`)
          const data = await response.json()
