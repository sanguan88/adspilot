...
import { checkResourceAccess, canAccessAll } from '@/lib/role-checker';
...
export async function GET(
  ...
  try {
    ...
    // Verify ownership using RBAC logic
    const canViewAll = canAccessAll(user.role, 'subscriptions');

    if (!canViewAll && transaction.user_id !== userId) {
      connection.release();
      return NextResponse.json(
        { success: false, error: 'Unauthorized: Cannot view another user invoice' },
        { status: 403 }
      );
    }
