...
async function getCampaignsFromDB(connection: PoolClient, startTime: string, endTime: string, user: UserPayload, tokoIds?: string[]) {
  try {
    ...
    // User isolation: Filter by user's toko unless user has permission to view all campaigns
    // Check if user has 'campaigns.view.all' permission
    const canViewAll = checkResourceAccess(user, 'view', 'campaigns', false); // false = check 'all' scope
    ...
    if (tokoIds && tokoIds.length > 0) {
      // For users without global view access, validate that tokoIds belong to them
      if (!canViewAll) {
