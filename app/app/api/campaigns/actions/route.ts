...
import { requirePermission, checkPermission } from '@/lib/role-checker'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // RBAC: Base permission check
    // Staff/Manager roles (who only have 'view' permissions) will be rejected here
    if (action === 'edit_budget') {
      requirePermission(user, 'campaigns.budget.own');
    } else if (action) {
      // pause, resume, stop, create_campaign
      requirePermission(user, 'campaigns.edit.own');
    }
    ...
    // If user does not have 'all' permission (e.g. not Admin/Superadmin), 
    // they must own the store they are modifying
    if (tokoId) {
      const isBudgetAction = action === 'edit_budget';
      const globalPerm = isBudgetAction ? 'campaigns.budget.all' : 'campaigns.edit.all';

      if (!checkPermission(user, globalPerm as any)) {
        const allowedTokos = await getAllowedUsernames(user);
        if (!allowedTokos.includes(tokoId)) {
          return NextResponse.json(
            { success: false, error: 'Access denied: You do not have permission to manage this store' },
            { status: 403 }
          );
        }
      }
    ...
    switch (action) {
      ...
      case 'create_campaign':
        ...
        if (daily_budget > 0) {
          ...
          // Budget validation: must be positive integer (no longer requires multiple of 500000)
          if (daily_budget < 0 || !Number.isInteger(daily_budget)) {
            return NextResponse.json(
              {
                success: false,
                error: 'Budget harus berupa bilangan bulat positif'
