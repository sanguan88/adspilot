...
// Function to call Shopee API get_time_graph using Python script (pertanggal)
async function callShopeeGetTimeGraphPython(cookies: string, date: string) {
...
start_timestamp = convert_date_to_timestamp(date, True)
end_timestamp = convert_date_to_timestamp(date, False)
...
`
    ...
    const pythonCommand = `python "${tempScriptPath}" "${escapedCookies}" "${date}"`
    ...
    try {
      ...
      console.error(`[sync-report-aggregate] Failed to cleanup temp file ${tempScriptPath}:`, cleanupError)
    }
  }
}
      ...
      for (const date of missingDates) {
        ...
        while (retryCount < maxRetries && !success) {
          try {
            ...
            const timeGraphData = await callShopeeGetTimeGraphPython(cookies, date)
