...
// Helper function untuk mengubah error API menjadi pesan yang user-friendly
function getUserFriendlyErrorMessage(apiErrors: string[]): string {
  // Check for common error patterns and provide friendly messages
  for (const error of apiErrors) {
    const lowerError = error.toLowerCase()

    // Cookie Too Large error
    if (lowerError.includes('400') && (lowerError.includes('cookie too large') || lowerError.includes('request header or cookie too large'))) {
      return 'Cookies terlalu panjang. Silakan clear cookies browser Anda, login ulang ke Shopee Seller Center, lalu copy cookies baru.'
    }

    // Cookies expired
    if (lowerError.includes('login=false') || lowerError.includes('cookies expired')) {
      return 'Cookies sudah expired. Silakan login ulang ke Shopee Seller Center dan copy cookies baru.'
    }

    // Network/timeout errors
    if (lowerError.includes('timeout') || lowerError.includes('econnrefused') || lowerError.includes('enotfound')) {
      return 'Gagal terhubung ke server Shopee. Silakan coba lagi dalam beberapa saat.'
    }

    // 401/403 errors
    if (lowerError.includes('401') || lowerError.includes('403') || lowerError.includes('unauthorized')) {
      return 'Cookies tidak valid atau sudah expired. Silakan login ulang ke Shopee Seller Center dan copy cookies baru.'
    }
    ...
    if (lowerError.includes('500') || lowerError.includes('502') || lowerError.includes('503')) {
      return 'Server Shopee sedang bermasalah. Silakan coba lagi dalam beberapa saat.'
    }
  }

  // Default message if no specific pattern matched
  return 'Gagal mengambil data toko dari Shopee. Pastikan cookies valid dan coba lagi.'
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filterCookies = searchParams.get('filter_cookies') || 'all' // Default to 'all' to show all accounts
    ...
    // Filter by cookies status
    if (filterCookies === 'connected') {
      // Only show accounts with status_cookies = 'aktif'
      whereConditions.push("(dt.status_cookies = 'aktif' OR (dt.status_cookies IS NULL AND dt.status_toko = 'active'))")
    } else if (filterCookies === 'disconnected') {
      // Only show accounts with expired cookies
      whereConditions.push("(dt.status_cookies = 'expire' OR dt.status_cookies = 'expired')")
    }
    // If filter_cookies = 'all', don't add any filter (show all accounts)
    ...
    // Query untuk mengambil data toko dengan pagination (include user_id and status_cookies)
    const offset = (page - 1) * limit
    const query = `SELECT dt.id_toko, dt.id_toko as username, dt.nama_toko, dt.username_toko, dt.email_toko, dt.profile_toko, dt.cookies, dt.status_toko, dt.status_cookies, dt.user_id, dt.created_at, dt.update_at
    ...
    for (const row of tokoData) {
      ...
      // Map status_cookies dan status_toko dari database ke cookie_status
      // Priority: status_cookies > status_toko
      let cookieStatus = 'no_cookies'

      // Check status_cookies first (more specific for cookie expiration)
      if (row.status_cookies) {
        const statusCookiesLower = row.status_cookies.toLowerCase()
        if (statusCookiesLower === 'aktif') {
          cookieStatus = 'connected'
        } else if (statusCookiesLower === 'expire') {
          cookieStatus = 'disconnected' // Expired cookies
        }
      } else if (row.status_toko) {
        // Fallback to status_toko if status_cookies is not set
        ...
        // Fallback: jika status_toko dan status_cookies null tapi ada cookies, anggap checking
      ...
      accountsWithPerforma.push({
        ...
        id_toko: row.id_toko, // Add id_toko field for consistency
        username: row.id_toko,
        email: row.email_toko || '',
        email_toko: row.email_toko || '',
        nama_toko: row.nama_toko || '',
        username_toko: row.username_toko || '',
        ...
        status_toko: row.status_toko || 'active', // Include status_toko
        status_cookies: row.status_cookies || null, // Include status_cookies
  ...
  try {
    ...
    if (action === 'add_toko') {
      ...
      try {
        ...
        try {
          ...
          if (loginResponse.ok) {
            ...
            // Check if login=false (cookies expired)
            if (loginData.login === false) {
              // Cookies expired - cannot proceed

              apiErrors.push(`API 2: login=false (cookies expired)`)
              // Set flag untuk expired
          ...
          // Check if it's expired cookies error
          if (err instanceof Error && err.message === 'COOKIES_EXPIRED') {
            releaseConnection()
            return NextResponse.json(
              {
                success: false,
                error: 'Cookies expired',
                message: 'Cookies yang diberikan sudah expired (login=false). Silakan gunakan cookies yang valid.'
              },
              { status: 400 }
            )
          }
        ...
        if (!shopid) {
          ...
          const errorMessage = apiErrors.length > 0
            ? getUserFriendlyErrorMessage(apiErrors)
      ...
      try {
        ...
        // First check login status to detect expired cookies
        let isLoginExpired = false
        try {
          const cleanCookies = account.cookies.trim().replace(/\s+/g, ' ').replace(/[\r\n\t]/g, '')
          const loginResponse = await fetch('https://seller.shopee.co.id/api/v2/login/', {
            method: 'GET',
            headers: {
              'Cookie': cleanCookies,
          ...
          if (loginResponse.ok) {
            ...
            if (loginData.login === false) {
              isLoginExpired = true
              // Update status_cookies to 'expire'
              await connection.query(
                'UPDATE data_toko SET status_cookies = $1 WHERE id_toko = $2',
                ['expire', idToko]
              )
            }
          }
        } catch (loginCheckError) {
          // Ignore login check error, continue with performa data check
        }
        ...
        try {
          // If login expired, skip performa data check
          if (isLoginExpired) {
          ...
          // Error saat request ke API
          apiError = true

          // Check if it's expired cookies error
          if (apiErr instanceof Error && apiErr.message === 'COOKIES_EXPIRED') {
            await connection.query(
              'UPDATE data_toko SET status_toko = $1 WHERE id_toko = $2',
              ['inactive', idToko]
            )
            ...
            return NextResponse.json({
              success: false,
              error: 'Cookies expired',
              message: `Cookies untuk toko ${idToko} sudah expired (login=false). Silakan update cookies.`,
              cookie_status: 'expired'
            }, { status: 400 })
          }

          // Other errors → update status_toko = 'inactive'
          await connection.query(
            'UPDATE data_toko SET status_toko = $1 WHERE id_toko = $2',
            ['inactive', idToko]
          )
          ...
          return NextResponse.json({
            ...
            cookie_status: cookieStatus || 'disconnected'
          }, { status: 400 })
        }
