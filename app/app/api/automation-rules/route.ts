...
import { requirePermission, checkResourceAccess } from '@/lib/role-checker'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // RBAC Check: Ensure user has permission to create rules
    // This will block 'staff' and 'manager' roles as per PERMISSIONS configuration
    try {
      requirePermission(user, 'rules.create');
    } catch (permissionError: any) {
      console.warn(`[${timestamp}] [Create Rule] Permission denied:`, permissionError.message);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // User isolation: Filter by user_id unless user has permission to view all
    const canViewAll = checkResourceAccess(user, 'view', 'rules', false); // false = check 'all' scope

    if (!canViewAll) {
      // User must either be the owner of the rule OR the rule must be assigned to one of their stores
      if (allowedUsernames.length > 0) {
        const placeholders = allowedUsernames.map(() => `$${paramIndex++}`).join(',')
        whereConditions.push(`(
          dr.user_id = $${paramIndex++} OR
          EXISTS (
            SELECT 1 FROM jsonb_object_keys(COALESCE(dr.campaign_assignments, '{}'::jsonb)) AS toko_id 
            WHERE toko_id IN (${placeholders})
          )
        )`)
        params.push(...allowedUsernames, user.userId)
      } else {
        // Fallback if no toko assigned to user: only show their own rules
