...
export async function GET(
  ...
  try {
    ...
    // Check if user has permission to view this specific rule
    // If owner: needs 'rules.view.own'
    // If not owner: needs 'rules.view.all'
    const hasViewPermission = isOwner
      ? checkPermission(user, 'rules.view.own')
      : checkPermission(user, 'rules.view.all');
    ...
    // Additional check for campaign assignments if not superadmin
    // Keeping this logic as it seems to be a specific business rule for shared access
    // If they have view.all (like Manager) allow them, otherwise check specific assignments
    if (!checkPermission(user, 'rules.view.all')) {
      let hasAccess = false
...
export async function PATCH(
  ...
  try {
    ...
    // If not owner, user MUST have 'rules.edit.all'
    // If owner, user MUST have 'rules.edit.own'
    if (isOwner) {
...
export async function DELETE(
  ...
  try {
    ...
    // If not owner, MUST have 'rules.delete.all'
    // If owner, MUST have 'rules.delete.own'
    if (isOwner) {
