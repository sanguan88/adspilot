  ...
  try {
    ...
    try {
      ...
      const result = await connection.query(
        ...
        AND status_user IN ('active', 'aktif', 'pending_payment')`,
        [username.trim()]
      );
      ...
      try {
        ...
        // Don't release connection here - it will be released after response
      }

      // Release connection setelah semua operasi selesai (hanya sekali)
      if (connection) {
        try {
          connection.release();
          connection = null; // Set to null to prevent double release
